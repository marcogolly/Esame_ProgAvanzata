...
/**
 * @brief Calcola il massimo comun divisore tra due numeri interi con
 * l'algortimo di Euclide
 *
...
private:
  int rows; //! è il numero di righe della matrice
  int cols; //! è il numero di colonne della matrice
  std::vector<std::vector<T>>
      matrix; //! è l'array 2D contenente gli elementi della matrice

public:
  /**
   * @brief Un costruttore: costruisce matrice nulla di dimensione rxc ( ogni
   * riga sarà un array di elementi)
   */
  Matrix(int r, int c) {
    rows = r;
    cols = c;
   ...
   */
  inline std::vector<T> &operator[](const size_t i) { return matrix[i]; }
  ...
  /**
   * @brief Calcola la matrice ridotta di una matrice con il metodo
   * dell'eliminazione di Gauss
   *
  ...
  Matrix<T> gauss() {
    ...
    for (int pivot_row = 0; pivot_col < rows - 1; ++pivot_row) {
      ...
      // se il pivot è nullo devo controllare che tutti gli elementi della
      // colonna i sotto di lui siano nulli
      if (res[pivot_row][pivot_col] == 0) {
        flag = false;

        for (int i = pivot_row; i < rows; ++i) {

          // se trovo una riga con un pivot valido, posso scambiare le righe
          // pivot_row e i a questo punto posso proseguire normalmente
      ...
      // se res[pivot_row][pivot_col] ==0 e non ho trovato una riga da scambiare
      // significa che dalla riga del pivot in giù, tutta la colonna pivot_col è
      // vuota, quindi passo alla colonna successiva
      if (!flag) {
        pivot_row--;
        pivot_col++;
        continue;
      ...
      for (int i = pivot_col + 1; i < rows; i++) {
        // nel caso delle matrici intere moltiplico la riga i per il pivot in
        // modo tale da poter sottrarre alla riga i un multiplo della riga
        // pivot_row garantendo di usare solo numeri interi
        if (std::is_integral<T>::value) {
          for (int j = 0; j < cols; ++j) {
            // questo algoritmo va spesso in overflow anche senza usare numeri
            // enormi, questo è inevitabile in alcuni casi siccome C++ non
            // gestisce gli overflow nelle operazioni tra interi, perciò lo
            // facciamo manualmente
            T max = std::numeric_limits<T>::max() / res[pivot_row][pivot_col];
            if (max < 0)
              max = -max;
            // se vado in overflow alzo un'eccezione
            if (res[i][j] > max) {
              std::cout << "questo " << res[i][j] << " è maggiore di " << max
                        << std::endl;
              throw std::overflow_error(
                  "Errore: impossible utilizzare gauss con questa matrice, "
                  "valori troppo grandi. Prova a convertire in un tipo più "
                  "grande (ad esempio longint oppure float");
            }
        ...
        if (std::is_integral<T>::value) {
          ...
          // nel caso di righe nulle max_div è 0, in quel caso lo poniamo uguale
          // a 1
          if (max_div == 0) {
            max_div = 1;
          }
  ...
  std::vector<T> mult_vect(const std::vector<T> &b) const {
    ...
    if (cols != b.size()) {
      throw std::invalid_argument(
          "Errore: il numero di colonne della matrice deve essere uguale al "
  ...
  void AddColumn(std::vector<T> &column) {
    // verifico che il vettore abbia lo stesso numero di elementi delle righe
    // della matrice
    if (column.size() != rows) {
      throw std::invalid_argument(
  ...
  /**
   * @brief Risolve un sistema lineare del tipo Ax=b attraverso l'eliminazione
   * di Gauss
   *
  ...
  std::vector<T> system(std::vector<T> &b) const {
    ...
    // controllo che il numero di elementi di b sia uguale al numero di righe
    // della matrice
    if (b.size() != rows) {
      throw std::invalid_argument(
    ...
    if (std::is_integral<T>::value) {
      ...
      // aggiungo il vettore dei termini noti alla matrice per creare la matrice
      // completa
      orig.AddColumn(b);
  ...
  Matrix<T> inverse() const {
    ...
    if (std::is_integral<T>::value) {
      ...
      // sottrai la riga corrente dalle righe sottostanti per annullare i
...
int main() {
  ...
  std::cout << "Tempo di esecuzione del metodo getRank(): " << duration.count()
